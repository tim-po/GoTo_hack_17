...
def handle_docs_audio(message):
    ...
    markup.add(menu)
    photo = add_corners(message.document
                        , rad=400)
...
def send_welcome(message):
    ...
    welcome_button4 = types.KeyboardButton('мастер на все руки')
...
message.text == 'участник' or message.text == 'преп' or message.text == 'орг' or message.text == 'мастер на все руки'
else False)
def send_message_token(message):
    ...
    if message.text == 'участник':
        ...
        bot.send_message(message.chat.id,
                         ...
                         reply_markup=markup)
    elif message.text == 'преп' or 'орг' or 'мастер на все руки':
...
message.text == 'преп token' or message.text == 'орг token' or message.text == 'мастер на все руки token'
else False)
def send_mesage(message):
    user_class = {'преп token': 'prep', 'орг token': 'org', 'мастер на все руки token': 'tvorog'}
    with open('db.json') as data_file:
        data = json.load(data_file)
    while data.get(str(message.from_user.id)) is not None:
        data.pop(str(message.from_user.id))
    ...
    if message.text == 'преп token':
        ...
        bot.send_message(message.chat.id, 'оке ты орг тебе доступны орг функции', reply_markup=markup2)
    elif message.text == 'мастер на все руки token':
        markup3 = types.ReplyKeyboardRemove(selective=False)
        bot.send_message(message.chat.id, 'оке ты мастер на все руки тебе доступны все функции', reply_markup=markup3)
    else:
...
def send_menu(message):
    ...
    next_menu = types.KeyboardButton('спец меню')
    ...
    if len(get_user(message.from_user.id)[0]) <= 2:
        markup.add(day_deals, next_menu, sticker)
...
@bot.message_handler(func=lambda message: True if
message.text in ['спец меню', 'настройка кастомных кнопок', 'название моей деректории',
...
def send_button_menu(message):
    ...
    if message.text == 'настройка кастомных кнопок':
        change_dir = types.KeyboardButton('название моей деректории')
        inner_button = types.KeyboardButton('внутреннюю кнопку 1')
        ...
        markup.add(menu, change_dir, inner_button, inner_button2, inner_button3, inner_button4)
        bot.send_message(message.chat.id, 'что надо настроить?',
                         reply_markup=markup)
    elif message.text == 'название деректории':
        bot.send_message(message.chat.id, 'напиши новое название своей деректории с пометкой ДЕР в начале сообщения',
                         reply_markup=markup)
    elif message.text[:-2] == 'внутреннюю кнопку':
        i2 = message.text[-1:]
        name = types.KeyboardButton('название кнопки ' + str(i2))
        funk = types.KeyboardButton('функционал кнопки ' + str(i2))
        markup.add(menu, name, funk)
        bot.send_message(message.chat.id, 'что ты хочешь изменить?',
                         reply_markup=markup)
